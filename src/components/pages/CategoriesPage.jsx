...
const CategoriesPage = ({ darkMode }) => {
  ...
  const fetchCategoryGames = async (categorySlug) => {
    setLoading(true)
    try {
      console.log('Fetching games for category:', categorySlug)
  ...
  const testAPI = async () => {
    try {
      const response = await fetch('https://api.rawg.io/api/games?key=4032143f42c84f39b7a79a56d7cb2e3d&page_size=1')
      const data = await response.json()
      ...
      <div className={`page-content ${darkMode ? 'dark' : 'light'}`}>
        <div className="category-header">
          ...
          <p className="games-count">
            {loading ? 'Loading...' : `${categoryGames.length} games found`}
          </p>
        </div>

        {loading ? (
          <div className="loading">Loading {selectedCategory.name} games...</div>
          ...
          <div className="no-games">
            No games found in {selectedCategory.name} category.
            <br />
            <button onClick={() => fetchCategoryGames(selectedCategory.slug)} className="retry-button">
              Try Again
            </button>
          </div>
        )}
